...
No import carries a file extension. The two aliases supply it (`"#lib/*": "./src/lib/*.ts"`), and a relative import is resolved without one, so `tsconfig.json` needs no `allowImportingTsExtensions`. `#assets/*` is the exception: it maps to `./assets/*` unchanged, since `#assets/index.json` names the extension itself.
...
- `src/components/` is every custom element, reached as `#components/*`. It imports from `src/lib/` and never from `src/routes/`.
- `src/routes/<slug>/` is one page: `index.html`, `index.css`, `index.ts`, and that route's own logic and tests. `src/routes/index.*` is the home pane. `src/routes/base.css` is the base stylesheet and belongs to no route.
