...
for (const section of SECTIONS) {
  ...
  for (const slug of declared) {
    if (!found.has(slug)) drift.push(`${section}/${slug} in sitemap, not parsed`);
  }
  for (const slug of found) {
    if (!declared.has(slug)) drift.push(`${section}/${slug} parsed, not in sitemap`);
  }
...
console.log(`done: ${done - failures.length} downloaded, ${failures.length} failed`);
